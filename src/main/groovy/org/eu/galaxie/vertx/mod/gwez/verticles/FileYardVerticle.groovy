...
class FileYardVerticle extends Verticle {
    ...
    def start() {
        ...
        [
                'gwez.fileYard.getBoardingPass': this.&getBoardingPass,
                'gwez.fileYard.onboardFile': this.&onboardFile,
                'gwez.fileYard.landFile': this.&landFile,
                'gwez.fileYard.calculateSha1': this.&calculateSha1,
                'gwez.fileYard.splitFile': this.&splitFile
    ...
    private void splitFile(Message message) {

        String fileToSplit = message.body.filename

        Integer chunkSize = message.body.chunkSize
...
println "splitting"
        ...
        message.reply([sha1: digestToString(digestOriginal), chunks: orderedParts])
    }
    ...
    private void onboardFile(Message message) {
        String target = message.body.filename
        println "onboarding ${target}"
        // TODO : check no '..' in target path, file existence relative to boardingDir, path is a regular file

        vertx.eventBus.send('gwez.fileYard.splitFile', [filename: target, chunkSize: (512 * 1024)]) { splitResponse ->
            message.reply(splitResponse.body)
        }
    }
    ...
    private void landFile(Message message) {
        ...
        // TODO : soft mode if file exists : do nothing but reply error
        if (newComer.exists()) {
            newComer.delete()
        }
